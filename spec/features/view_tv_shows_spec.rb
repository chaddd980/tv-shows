...
feature "user views list of TV shows" do
  ...
  scenario "view details for a TV show" do
    ...
    chappelle_show = TelevisionShow.create!({
        title: "Chappelle Show", network: "Comedy Network",
        starting_year: 2005, ending_year: 2007, genre: "Comedy"
      })

    visit "/television_shows/#{chappelle_show.id}"
  ...
  scenario "view details for a TV show with missing information" do
    ...
    westworld = TelevisionShow.create!({
      ...
      visit "/television_shows/#{westworld.id}"
      expect(page).to have_content("Network:")
      expect(page).to have_content("Years:")
      expect(page).to have_content("Genre:")
